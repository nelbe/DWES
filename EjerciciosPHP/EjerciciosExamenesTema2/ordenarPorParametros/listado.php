  ...
  <body>
    ...
    //Hago la conexion desde otro fichero, con un include
      ...
      <form action="#" method="post">
        ...
        if($selected_val == codigo){
            $consulta = mysql_query("SELECT * FROM articulo ORDER BY codigo LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }elseif($selected_val == stock){
            $consulta = mysql_query("SELECT * FROM articulo ORDER BY stock LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }elseif($selected_val == precio_venta){
            $consulta = mysql_query("SELECT * FROM articulo ORDER BY precio_venta LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }else{
            $consulta = mysql_query("SELECT * FROM articulo ORDER BY precio_venta LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }
