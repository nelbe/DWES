  ...
  <body>
    ...
    //Hago la conexion desde otro fichero, con un include
      ...
      <form action="#" method="post">
        ...
        <input type="submit" name="submit" value="Ordenar Por:" />
      </form>
        <?php
        if(isset($_POST['submit'])){
        $selected_val = $_POST['ordenar'];  // Storing Selected Value In Variable
        }
        
        if($selected_val == interior){
            $consulta = mysql_query("SELECT * FROM articulo WHERE categoria = 'interior' LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }elseif($selected_val == exterior){
            $consulta = mysql_query("SELECT * FROM articulo WHERE categoria = 'exterior' LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }elseif($selected_val == todos){
            $consulta = mysql_query("SELECT * FROM articulo ORDER BY codigo LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }
        else{
            $consulta = mysql_query("SELECT * FROM articulo LIMIT " . (5 * ($_SESSION['pagina']-1)) . ",5", $conexion);
        }
    ...
    <form action="carrito.php" method="post">
        <input id="venta" type="submit" value="venta">     
    </form>
